//The initialize function takes a single parameter that is a reference to the element the map should be rendered in
export function initialize(hostElement, routeMapComponent, existingWaypoints, isReadOnly) {
    //Leaflet is initialized on the hostElement
    hostElement.map = L.map(hostElement).setView([51.700, -0.10], 3);

    //Adds a layer that displays a copyright message in the bottom right of the map
    ...
    //This hooks up a handler for the click event exposed by the map.
    //When the map is clicked, the handler will add a waypoint and if there is more than one waypoint, add a line connecting them
    if (!isReadOnly) {
